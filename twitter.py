...
class Twitter:
    ...
    def get_tweets_by_poi_screen_name(self,keyword,lang,count):
        ...
        '''
          _list=[]
        for tweet in tweepy.cursor(self.api.user_timeline,screen_name=poiName,tweet_mode='extended').items(10):
            _list.append(tweet)
        return _list
    ...
    def get_replies(self,id_str,screen_name):
        '''
        ...
        '''
          _list=[]
        for tweet in tweepy.cursor(self.api.search,q='to:'+user,since_id=id_str timeout=999999).items(maxID):
